...
# SUA SOLUÇÃO AQUI Leonam De Sousa Silva ------------------------------------------------------------------------------
...
# SUA SOLUÇÃO AQUI ↓↓↓ Cauê Soares Raposo
import json
...
# SUA SOLUÇÃO AQUI Leonam De Sousa Silva ------------------------------------------------------------------------------
desconto_registry = {
    "cupom:blackfriday":       lambda c: (0.30, "Cupom Black Friday: 30%"),
    "cupom:bemvindo:valido":   lambda c: (0.10, "Cupom de boas-vindas: 10%"),
    "cupom:bemvindo:invalido": lambda c: (0.0, "Cupom BEMVINDO10 apenas para novos clientes"),
    "cliente:vip:alto_valor":  lambda c: (0.20, "VIP + compra >= R$500: 20%"),
    "cliente:vip:padrao":      lambda c: (0.10, "VIP: 10% base"),
    "cliente:fiel:muitos_itens": lambda c: (0.15, "Fiel + 5 itens: 15%"),
    "cliente:fiel:padrao":      lambda c: (0.08, "Fiel: 8%"),
    "fds:alto_valor":          lambda c: (0.12, "FDS + >= R$200: 12%"),
    "fds:padrao":              lambda c: (0.05, "Fim de semana: 5%"),
    "compra:acima_mil":        lambda c: (0.10, "Compra acima de R$1.000: 10%"),
    "aniversario_loja":        lambda c: (0.25, "Aniversário da Loja! Quarta-feira Especial: 25%"), # Desafio!
    "padrao":                  lambda c: (0.0, "Sem desconto aplicável")
}

def decidir_regra(carrinho: Carrinho) -> str:
    """O Decision Engine — Isolando apenas o roteamento estrutural."""
    if carrinho.dia_semana == 3 and carrinho.valor_total >= 300: # Avaliação da nova regra
...
def calcular_desconto_depois(carrinho: Carrinho) -> tuple[float, str]:
    chave = decidir_regra(carrinho)
    handler = desconto_registry.get(chave, desconto_registry["padrao"])
    return handler(carrinho)
...
# Teste rápido
carrinho_niver = Carrinho(350.0, None, "normal", 3, 2) # Quarta-Feira, valor >= 300
pct, motivo = calcular_desconto_depois(carrinho_niver)
print(f"  ⚡ Desafio -> R$ {carrinho_niver.valor_total:7.2f} | {pct*100:4.0f}% | {motivo}")
...
# Teste rápido do Desafio da Atividade 4
carrinho_niver = Carrinho(350.0, None, "normal", 3, 2) # Quarta-Feira, valor >= 300
pct, motivo = calcular_desconto_depois(carrinho_niver)
print(f"  ⚡ Desafio -> R$ {carrinho_niver.valor_total:7.2f} | {pct*100:4.0f}% | {motivo}")
